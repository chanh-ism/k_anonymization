...
from ..algorithms.utils import generalize_df
...
class MLClassificationMetricsGeneralizationCSEC112:
    def __init__(
        self,
        model,
        base_validation_df,
        ...
        self.cat_features = (
            cat_features
            if cat_features != []
            else [x for x in list(_df) if x != self.target]
        )
        self.label_encoder = LabelEncoder()
        self.base_validation_df = base_validation_df
        self.y_validation = self.label_encoder.fit_transform(base_validation_df[target])

    def __generalize_validation_data(
        self, validation_data, hierarchies, generalization_level: dict
    ):
        generalized_data = validation_data[:]
        for att, level in generalization_level.items():
            if level == 0:
                continue
            if level == -1:
                generalized_data.drop(columns=[att], inplace=True)
                continue
            for l in range(level):
                generalized_data, _ = generalize_df(
                    generalized_data, hierarchies[att], l
                )
        return generalized_data

    def update_df(self, df, generalization_level=None, hierarchies=None):

        self.one_hot_encoder = OneHotEncoder(
            sparse_output=False, handle_unknown="ignore"
        )

        X_org = df[self.cat_features].drop(
            columns=[att for att, level in generalization_level.items() if level == -1]
        )
        self.org_data = X_org

        self.X = DataFrame(
            self.one_hot_encoder.fit_transform(X_org),
            columns=self.one_hot_encoder.get_feature_names_out(),
        )
        self.y = self.label_encoder.fit_transform(df[self.target])
        ...
        self.set_X_validation(generalization_level, hierarchies)

    def set_X_validation(self, generalization_level=None, hierarchies=None):
        X_org = self.__generalize_validation_data(
            self.base_validation_df[self.cat_features],
            hierarchies,
            generalization_level,
        )

        self.X_validation = DataFrame(
            self.one_hot_encoder.transform(X_org),
            columns=self.one_hot_encoder.get_feature_names_out(),
        )
        self.y_pred_validation = None
